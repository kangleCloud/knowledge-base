...
在编译安装 zabbix server 或 zabbix proxy 服务时，已经编译安装了 Zabbix JavaGateway，只需要配置 JavaGateway 即可使用 JMX
监控。

## 修改配置文件

```bash
vim /usr/local/zabbix6.0/sbin/zabbix_java/setting.sh
```
...
```bash
# 启动
/usr/local/zabbix6.0/sbin/zabbix_java/startup.sh

# 关闭
/usr/local/zabbix6.0/sbin/zabbix_java/shutdown.sh
```
...
- https://www.zabbix.com/documentation/6.0/zh/manual/concepts/java/from_sources
