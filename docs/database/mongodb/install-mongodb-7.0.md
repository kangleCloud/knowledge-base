...
[截止2025年8月23日，MongoDB 7.0 最新稳定版为 7.0.23。](https://www.mongodb.com/zh-cn/docs/upcoming/release-notes/7.0/)

<font color="red"><b>每次安装需到官网查看 MongoDB 7.0 系列的最新版。</b></font>

:::tip
- 所有源码包下载到 /usr/local/src 中
- 软件安装到 /usr/local 中，并以软件名及主次版本号命名，如 mongodb7.0
:::
...
:::warning
[如果打开文件数的 ulimit 值低于 64000，MongoDB 会生成初创企业警告。](https://www.mongodb.com/zh-cn/docs/v7.0/tutorial/install-mongodb-on-red-hat-tarball/#ulimit)
:::
...
1. In the **Version** dropdown, select the version of MongoDB to download. => **7.0.***

2. In the **Platform** dropdown, select your operating system version and architecture. => **RedHat / CentOS 8.0 x64**
...
tar xvf mongodb-linux-x86_64-rhel8-7.0.23.tgz -C /usr/local/
mv /usr/local/mongodb-linux-x86_64-rhel80-7.0.23  /usr/local/mongodb7.0
```
---
:::
...
```bash
echo 'export PATH=/usr/local/mongodb7.0/bin:$PATH' >> /etc/profile
source /etc/profile
```
...
[自管理的配置文件选项](https://www.mongodb.com/zh-cn/docs/v6.0/reference/configuration-options/#configuration-file-options)

:::tip
- [如果通过 TGZ 或 ZIP 文件安装 MongoDB，则需要创建自己的配置文件。](https://www.mongodb.com/zh-cn/docs/v7.0/administration/configuration/#std-label-base-config)
:::

1. 创建配置文件目录

```bash
mkdir -pv /usr/local/mongodb7.0/conf
```

2. 创建并编辑配置文件
   
```bash
vim /usr/local/mongodb7.0/conf/mongod.conf
```
...
User=mongod
Group=mongod
Environment="OPTIONS=-f /usr/local/mongodb7.0/conf/mongod.conf"
ExecStart=/usr/local/mongodb7.0/bin/mongod $OPTIONS
ExecStop=/usr/local/mongodb7.0/bin/mongod $OPTIONS --shutdown
LimitNOFILE=65535
...
- https://www.mongodb.com/zh-cn/docs/v7.0/tutorial/manage-mongodb-processes
...
https://www.mongodb.com/zh-cn/docs/v7.0/tutorial/transparent-huge-pages
...
db.news_article.insertOne({title: '稿件标题'})  #写入数据
```
...
- [在 Red Hat 或 CentOS 上安装 MongoDB Community Edition](https://www.mongodb.com/zh-cn/docs/v7.0/tutorial/install-mongodb-on-red-hat/)
- [使用 .tgz 在 Red Hat 或 CentOS 上安装 MongoDB Community Edition](https://www.mongodb.com/zh-cn/docs/v7.0/tutorial/install-mongodb-on-red-hat-tarball/)